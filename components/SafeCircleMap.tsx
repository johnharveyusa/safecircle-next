...
export default function SafeCircleMap({ center, crimes, offenders }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <Popup><strong>Searched address</strong></Popup>
      </CircleMarker>
      ...
      {crimes
        ...
        .map((c) => (
          ...
          >
            <Popup>
              <strong>{c.offense_type}</strong><br />
              {c.address}<br />
          ...
          return (
            ...
            >
              <Popup>
                ...
                {o.offenseDescription}<br />
                <span style={{ color: "#A32D2D", fontSize: 11 }}>
                  {o.tier ? `Tier ${tierKey}` : ""}
                  {o.distanceMi != null ? ` · ${o.distanceMi} mi` : ""}
                </span>
                {o.registryUrl && (
                  <><br /><a href={o.registryUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11 }}>View registry record ↗</a></>
                )}
              </Popup>
            </CircleMarker>
          );
